...
namespace geom {
	...
	public:
		...
		constexpr T magnitude() const noexcept { return std::sqrt(magnitude2()); }
		...
		constexpr Vec2 perpCCW() const noexcept { return Vec2{-y, x}; }
		// Clockwise (non-normalized) normal to the vector (rotate 90 degrees clockwise).
		constexpr Vec2 perpCW() const noexcept { return Vec2{y, -x}; }

		// Project along projDir by dist (use dist if this is a unit vector).
		constexpr Vec2 project(const Vec2& projDir, const T dist = {0}) const noexcept {
